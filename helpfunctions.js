...
function loadExistingReactionToAdd(gp) {
    ...
    var metfield = document.getElementById("metNameOpts").value;
    for (j in gp) {
        for (var i = 0; i < gp[j].nodes.length; i++) {
            if (gp[j].nodes[i].group == 1 && allrxns.indexOf(gp[j].nodes[i][rxnfield]) == -1) {
                allrxns.push(gp[j].nodes[i][rxnfield])
            } else if (gp[j].nodes[i].group == 2 && allmets.indexOf(gp[j].nodes[i][metfield]) == -1) {
                allmets.push(gp[j].nodes[i][metfield])
            }
        }
    }
...
function addExistingReaction(e) {
    if (e.keyCode == 13) {
        ...
        setTimeout(function() {
            ...
            for (j in parsedmodels) {
                for (var i = 0; i < parsedmodels[j].nodes.length; i++) {
                    if (parsedmodels[j].nodes[i][rxnfield] == rx) {
            ...
            if (nodeadded) {
                ...
                for (var i = 0; i < limit; i++) {
                    if (parsedmodels[j].links[i].source[rxnfield] == rx) {
                    ...
                    } else if (parsedmodels[j].links[i].target[rxnfield] == rx) {
                        var newlink = Object.assign(JSON.parse(JSON.stringify(parsedmodels[j].links[i])),{
                ...
                for (var i = 0; i < parsedmodels[j].suspended.length; i++) {
                    for (var k = 1; k < (parsedmodels[j].suspended[i].length-1); k++) {
                        ...
                        if (tmp[0] == graph.nodes[newnodeindex][rxnfield]) {
            ...
            reDefineSimulation()
            document.getElementById("existingReactions").value = "";
        },100)
    }
}
...
var tmp
function getPdf() {
    //Add circle to selected nodes
    node.filter(function(d){return d.selected}).attr("stroke","black")
    ...
    //Output svg
    toprect = gMain.append('rect')
    .attr('width', $(window).width())
    .attr('height', imgh+imgo)
    .attr("x", 0)
    .attr("y", -imgh-imgo)
    .style('fill', 'white')

    var x = document.getElementById("graphsvg");

    var width = Number(x.attributes.width.value), height = Number(x.attributes.height.value);
    const pdf = new jsPDF('l', 'pt', [width, height+imgh+imgo]);

    var imgData;
    toDataUrl('Thumbnails/MDACC.png', function(base64Img){
        imgData =  base64Img;
        })
    ...
    setTimeout(() => {
        ...
        if (document.getElementById(id="savepdfcs").checked){
            //Add Page
            pdf.addPage();
            ...
            //buil colorbar svg
            var y = document.createElement('svg');
            y.width = width; y.height = height;
            
            //Reaction color-scale
            step = (fluxmax - fluxmin) / 255;
            for(var i = 0; i < 254; i++) {
                colorvec = hexToRgb(defineFluxColor(fluxmin + i*step))
                var color = 'rgb(' + colorvec[0] + ', ' + colorvec[1] + ', ' + colorvec[2] + ')';
                var rect = document.createElement('rect');
                rect.setAttribute("x",25+i)
            ...
            colorvec = hexToRgb(defineFluxColor(fluxmax))
            var color = 'rgb(' + colorvec[0] + ', ' + colorvec[1] + ', ' + colorvec[2] + ')';
            var rect = document.createElement('rect');
            rect.setAttribute("x",25+i)
            rect.setAttribute("y",25)
            ...
            for (var j = 0; j < rxncolorbreaks.length; j++) {
                ...
                text.setAttribute("x",(255*(rxncolorbreaks[j]-fluxmin)/(fluxmax-fluxmin))+25)
                text.setAttribute("y",62)
                text.textContent = rxncolorbreaks[j];
                y.appendChild(text)
            }
            var text = document.createElement('text');
            text.setAttribute("x",290)
            text.setAttribute("y",40)
            text.textContent = 'Reaction Color';
            y.appendChild(text)

            //Metabolite color-scale
            step = (concentrationmax - concentrationmin) / 255;
            for(var i = 0; i < 254; i++) {
                colorvec = hexToRgb(defineFluxColorMet(concentrationmin + i*step))
                var color = 'rgb(' + colorvec[0] + ', ' + colorvec[1] + ', ' + colorvec[2] + ')';
            ...
            colorvec = hexToRgb(defineFluxColorMet(concentrationmax))
            var color = 'rgb(' + colorvec[0] + ', ' + colorvec[1] + ', ' + colorvec[2] + ')';
            var rect = document.createElement('rect');
            rect.setAttribute("x",25+i)
            rect.setAttribute("y",90)
            ...
            for (var j = 0; j < metcolorbreaks.length; j++) {
                var text = document.createElement('text');
                text.setAttribute('text-anchor','middle')
                text.setAttribute("x",(255*(metcolorbreaks[j]-concentrationmin)/(concentrationmax-concentrationmin))+25)
                text.setAttribute("y",127)
                text.textContent = metcolorbreaks[j];
                y.appendChild(text)
            }
            var text = document.createElement('text');
            text.setAttribute("x",290)
            text.setAttribute("y",105)
            text.textContent = 'Metabolite Color';
            y.appendChild(text)
            ...
            rs = [minmetsize*Number(document.getElementById("metsizescale").value)*pos,
                ...
                maxrxnsize*Number(document.getElementById("rxnsizescale").value)*pos]
            rso = [minmetsize + " - Metabolite Node Size",maxmetsize,
            minrxnsize + " - Reaction Node Size",maxrxnsize]
            mr = Math.max(...rs);
            sf = 170;
            ...
            for (var i = 0; i < 4; i++) {
                ...
                y.appendChild(circle)

                var text = document.createElement('text');
                text.setAttribute("x",2*mr+20)
                text.setAttribute("y",ys[i])
                text.setAttribute("alignment-baseline","middle")
                text.textContent = rso[i];
                y.appendChild(text)
            }

            //Draw Lines
            str = ys[3]+rs[3]+20;
            sminw = minwidth*document.getElementById("widthscale").value*pos;
            smaxw = maxwidth*document.getElementById("widthscale").value*pos;
            path = document.createElement('path')
            path.setAttribute("d","M" + 5 + " " + (str+(sminw/2)) + "L" + (2*mr+5) + " " + (str+(sminw/2)))
            path.setAttribute("stroke-width",sminw)
            path.setAttribute("stroke","black")
            y.appendChild(path)

            path = document.createElement('path')
            path.setAttribute("d","M" + 5 + " " + (str+sminw+(smaxw/2)+10) + "L" + (2*mr+5) + " " + (str+sminw+(smaxw/2)+10))
            path.setAttribute("stroke-width",smaxw)
            path.setAttribute("stroke","black")
            y.appendChild(path)

            var text = document.createElement('text');
            text.setAttribute("x",2*mr+20)
            text.setAttribute("y",str+(sminw/2))
            text.setAttribute("alignment-baseline","middle")
            text.textContent = minwidth + " - Link Width";
            y.appendChild(text)

            var text = document.createElement('text');
            text.setAttribute("x",2*mr+20)
            text.setAttribute("y",str+sminw+(smaxw/2)+10)
            text.setAttribute("alignment-baseline","middle")
        ...
        //Return stroke
        node.filter(function(d){return d.selected}).attr("stroke","white")
    },500)
}
...
function toDataUrl(src, callback, outputFormat) {
  ...
  savePNG = () =>  {
    ...
    var a = document.createElement('a');
        a.setAttribute('download', 'SAMMI.png');
    ...
    document.getElementById("blankdiv").appendChild(canvas)
    ...
    var logo = new Image();
    logo.src = "Thumbnails/MDACC.png"

    img.onload = function () {
        ctx.drawImage(img, 0, 0);
        ctx.drawImage(logo, 10, 10, 240, 115);
        DOMURL.revokeObjectURL(url);

        var imgURI = canvas
            .toDataURL('image/png',10)
            .replace('image/png', 'image/octet-stream');
        setTimeout(() => triggerDownload(imgURI),1000)
    };
    img.src = url;
    //canvas.remove()
    centerref.attr("r",document.getElementById("centersize").valueAsNumber)
    cover.remove()
}
